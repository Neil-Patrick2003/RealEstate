export default function FloatingInput({
  id, label, value, onChange, name, type = "text", required = true
}) {
  return (
    <label htmlFor={id} className="relative block w-full">
      <input
        id={id}
        name={name}
        type={type}
        required={required}
        value={value}
        onChange={onChange}
        placeholder=" "
        className="peer w-full border border-gray-200 rounded-md px-4 py-3 text-sm text-gray-800 bg-white placeholder-transparent transition focus:outline-none focus:ring-2 focus:ring-emerald-200 focus:border-emerald-500"
      />
      <span
        className="absolute left-4 top-2 bg-white px-1 text-sm text-gray-500 uppercase tracking-wide transition-all duration-200 pointer-events-none
          peer-placeholder-shown:top-2
          peer-placeholder-shown:text-sm
          peer-focus:text-sm
          peer-focus:top-[-10px]
          peer-focus:text-emerald-600
          peer-valid:text-sm
          peer-valid:top-[-10px]"
      >
        {label}
      </span>
    </label>
  );
}
