...
export default function ContactBroker({ broker, auth, setIsOpenModal, setSelectedPerson, setData }) {

    const handleContactClick = () => {
        setSelectedPerson(broker);   // set first
        setData('person', broker.id);   // <-- important
        setIsOpenModal(true);        // then open modal
    };

    return (
        <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-4">Property Broker</h2>
            <div className="flex flex-col space-y-4">
                <div key={broker.id} className="bg-white border rounded-xl shadow-sm p-6 mb-6 sticky top-6">
                    <div className="flex items-center mb-4">
                        {broker.photo_url ? (
                            <img
                                src={`/storage/${broker.photo_url}`}
                    ...
                    <button
                        ...
                        className="w-full mt-4 px-4 py-2 rounded-lg transition-colors font-medium flex items-center justify-center bg-primary hover:bg-accent text-white disabled:opacity-50 disabled:cursor-not-allowed"
                        title={
                            !auth
                                ? 'You must be logged in to contact a broker'
                                : auth.role !== 'Buyer'
                                    ? 'Only buyers can contact brokers'
                                    : 'Contact the broker'
                        }
                    >
